...
export const reducerUtils = {
    ...
    //로딩 상태. prevState는 기본적으로 null이지만 따로 지정하면 다른 값으로 유지시킬 수 O
...
//비동기 관련 액션들을 처리하는 리듀서를 만들어 주는 함수
//type은 액션 타입, key는 상태의 key(예: posts,post)
//재로딩 피하는 방법2 keepdData=false
...
export const createPromiseThunkById = (
    type,
    promiseCreator,
    // 파라미터에서 id 를 어떻게 선택 할 지 정의하는 함수
    // 기본 값으로는 파라미터를 그대로 id로 사용
    // 하지만 만약 파라미터가 { id: 1, details: true } 이런 형태라면
    // idSelector 를 param => param.id 이런식으로 설정 할 수 O
